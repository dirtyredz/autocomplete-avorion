...
let suggestions = enums.concat(functions,objects,callbacks)
...
class AdvancedProvider {
	...
	getSuggestions(options) {
		...
		if (prefix.delim && prefix.delim.endsWith(':')) {
			let leftSuggestion = this.findExactSuggestions(suggestions, prefix.left);
		...
		if (prefix.delim && prefix.delim.endsWith('.')) {
			...
			if (!leftSuggestion) { return null }
			if (!leftSuggestion.inherited && !leftSuggestion.properties) { return null }
	...
	findExactSuggestions(SuggestionGroup, prefix) {
		let matches = this.findMatchingSuggestions(SuggestionGroup, prefix);
		let matched;
		matches.forEach((x)=>{ if(x.text==prefix){ matched = x; } });
		return matched;
	}
	...
	inflateSuggestion(suggestion,leftSuggestion) {
		...
		if(suggestion.type == 'enum'){
			...
			icon = 'P';
		}else if(suggestion.type == 'callback'){
			icon = 'C';
		}
		return {
